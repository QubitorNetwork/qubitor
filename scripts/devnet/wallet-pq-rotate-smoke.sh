...
if [[ ! -d "$WALLET_DIR" ]]; then
  echo "[qubitor-wallet-pq-rotate] missing wallet repo: $WALLET_DIR" >&2
  exit 1
fi

if ! command -v pnpm >/dev/null 2>&1; then
  echo "[qubitor-wallet-pq-rotate] pnpm is required" >&2
  exit 1
fi
...
(
  ...
  QUBITOR_MOBILE_ROTATE_ACCEPTANCE_TARGET="${QUBITOR_WALLET_PQ_ROTATE_SMOKE_TARGET:-0x000000000000000000000000000000000000dEaD}" \
  QUBITOR_MOBILE_ROTATE_ACCEPTANCE_VALUE_WEI="${QUBITOR_WALLET_PQ_ROTATE_SMOKE_VALUE_WEI:-54321}" \
  pnpm --filter @qubitor/mobile devnet:rotate-acceptance
)

echo "[qubitor-wallet-pq-rotate] ok"
