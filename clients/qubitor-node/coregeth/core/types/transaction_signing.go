...
var (
	...
	ErrInvalidQubitorPQAuth = errors.New("invalid qubitor pq transaction authorization")
)
...
func (s eip4844Signer) Sender(tx *Transaction) (common.Address, error) {
	if tx.Type() == QubitorPQTxType {
		return qubitorPQSender(s.chainId, tx)
	}
...
func qubitorPQHash(tx *Transaction) common.Hash {
	if qtx, ok := tx.inner.(*QubitorPQTx); ok {
		return qtx.signingHash()
	}
	return common.Hash{}
}

func qubitorPQSender(chainID *big.Int, tx *Transaction) (common.Address, error) {
	qtx, ok := tx.inner.(*QubitorPQTx)
	if !ok {
		return common.Address{}, ErrTxTypeNotSupported
	}
	if chainID == nil {
	...
	if qtx.ChainID == nil || qtx.ChainID.Cmp(chainID) != 0 {
		have := "<nil>"
		if qtx.ChainID != nil {
			have = qtx.ChainID.String()
		}
		return common.Address{}, fmt.Errorf("%w: have %s want %d", ErrInvalidChainId, have, chainID)
	}
	if qtx.Account == (common.Address{}) || len(qtx.PQPublicKey) == 0 || len(qtx.PQSignature) == 0 {
		return common.Address{}, ErrInvalidQubitorPQAuth
	}
	if qtx.Account != QubitorPQAccountAddress(qtx.PQPublicKey, qtx.FactorySalt) {
		return common.Address{}, ErrInvalidQubitorPQAuth
	}

	var publicKey circlmldsa65.PublicKey
	if err := publicKey.UnmarshalBinary(qtx.PQPublicKey); err != nil {
		return common.Address{}, ErrInvalidQubitorPQAuth
	}
	hash := qtx.signingHash()
	if !circlmldsa65.Verify(&publicKey, hash.Bytes(), qtx.pqContext(), qtx.PQSignature) {
		return common.Address{}, ErrInvalidQubitorPQAuth
	}
	return qtx.Account, nil
}
