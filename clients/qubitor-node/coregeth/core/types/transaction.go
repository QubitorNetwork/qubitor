...
// QubitorPQAccount returns the Qubitor wallet/account sender embedded in a
// Qubitor-native transaction.
func (tx *Transaction) QubitorPQAccount() (common.Address, bool) {
	if qtx, ok := tx.inner.(*QubitorPQTx); ok {
		return qtx.Account, true
	}
	return common.Address{}, false
}

// QubitorPQAccountAddress derives the canonical Qubitor-native account address
// for an ML-DSA public key and factory salt.
func QubitorPQAccountAddress(publicKey []byte, factorySalt common.Hash) common.Address {
	return qubitorAccountCreate2Address(publicKey, factorySalt)
}

// QubitorPQSigningHash returns the ML-DSA signing hash for a Qubitor-native
// transaction.
func (tx *Transaction) QubitorPQSigningHash() (common.Hash, bool) {
	if qtx, ok := tx.inner.(*QubitorPQTx); ok {
		return qtx.signingHash(), true
	}
