...
func ValidateTransaction(tx *types.Transaction, head *types.Header, signer types.Signer, opts *ValidationOptions) error {
	...
	if os.Getenv("QUBITOR_EOA_TXS") == "0" && tx.Type() != types.QubitorPQTxType {
		return fmt.Errorf("%w: legacy Ethereum transaction type %v disabled by QUBITOR_EOA_TXS=0", core.ErrTxTypeNotSupported, tx.Type())
	}
